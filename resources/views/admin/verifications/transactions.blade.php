...
<div class="page-header d-print-none mb-4">
    <div class="container-xl">
        <div class="row g-2 align-items-center">
            <div class="col">
                <div class="page-pretitle text-uppercase fw-bold text-blue" style="letter-spacing:.1em;">Admin</div>
                <h2 class="page-title fw-black">Verifikasi Transaksi Agen</h2>
                <div class="text-muted small">Transaksi yang diajukan oleh agen atau dibuat oleh admin.</div>
            </div>
...
<div class="container-xl">
    ...
    <ul class="nav nav-tabs mb-4">
        ...
        @foreach($tabs as $value => $label)
            <li class="nav-item">
                <a class="nav-link {{ $statusFilter === $value ? 'active fw-bold' : '' }}"
                   href="{{ route('admin.verifications.transactions', $value !== 'all' ? ['status' => $value] : []) }}">
                    {{ $label }}
                    @if($value === 'pending' && $pendingCount > 0)
                        <span class="badge bg-warning ms-1 text-warning-fg">{{ $pendingCount }}</span>
                    @endif
                </a>
            </li>
        @endforeach
            ...
            <table class="table card-table table-vcenter text-nowrap datatable">
                <thead>
                    <tr>
                        ...
                        <th>Bukti Bayar</th>
                        <th>Status</th>
                        <th>Reviewer / Verifikator</th>
                        <th class="d-print-none text-end">Aksi</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            @if($txn->verifier)
                                <span>{{ $txn->verifier->username }}</span><br>
                                <small class="text-muted">{{ $txn->verified_at ? $txn->verified_at->format('d/m H:i') : '' }}</small>
                            @else
                            ...
                            @if($txn->status->value === 'pending')
                                <div class="d-flex gap-2 justify-content-end">
                                    <form action="{{ route('admin.transactions.approve', $txn->id) }}" method="POST">
                                        ...
                                        <button class="btn btn-sm btn-success"
                                            onclick="return confirm('Peringatan: Konfirmasi pembayaran valid? Ini akan meneruskan transaksi ke Superadmin untuk persetujuan akhir.');">
                                            Valid
                                        </button>
                                    </form>
                                ...
                                @if($txn->type->value === 'new_agent' && $txn->agent->no_telp)
                                    @php
                                        ...
                                        $waMessage = urlencode("Halo {$txn->agent->nama}, selamat bergabung di Sahihbodyfeed! Pendaftaran Anda sedang direview oleh admin. 🎉");
                                        $waUrl = "https://wa.me/{$waNumber}?text={$waMessage}";
                                    @endphp
                                    <button type="button" class="btn btn-primary btn-sm mt-2 w-100" onclick="window.open('{{ $waUrl }}', '_blank');">
                                ...
                                @endif
                            @elseif($txn->status->value === 'pending_superadmin')
                                <span class="small text-azure fw-bold">⏳ Menunggu Superadmin</span>
